...
func (client *TransparentEndpointClient) DeleteEndpointRules(ep *endpoint) {
	...
	for _, ipAddr := range ep.IPAddresses {
		...
		routeInfo.Dst = ipNet
		if err := deleteRoutes(client.netlink, client.hostVethName, []RouteInfo{routeInfo}); err != nil {
...
func (client *TransparentEndpointClient) setupIPV6Routes(epInfo *EndpointInfo) error {
	if epInfo.IPV6Mode != "" {
		...
		if err := addRoutes(client.netlink, client.containerVethName, []RouteInfo{gwRoute, defaultRoute}); err != nil {
			return err
		}
